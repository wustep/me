...
export default function Game() {
  ...
  const [grid, _setGrid] = useState<string[][]>(
    new Array(GRID_SIZE)
      .fill(null)
      .map(() => new Array(GRID_SIZE).fill(CELL_EMPTY))
  )
  ...
  const placeBomb = useCallback(
    (player: 'p1' | 'p2') => {
      ...
      const explodeBomb = (player: 'p1' | 'p2', bomb: Bomb) => {
        ...
        for (const [ex, ey] of explosionCoords) {
          if (
        ...
        for (const { player, bomb } of triggeredBombs) {
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      ...
      for (const control of PLAYER_1_CONTROLS) {
        if (keys[control.key]) {
      ...
      for (const control of PLAYER_2_CONTROLS) {
        if (keys[control.key]) {
      ...
      setExplosions((prev) => {
        for (const explosion of prev) {
          for (const [ex, ey] of explosion.coords) {
            const currentTime = Date.now()
        ...
        if (expired.length > 0) {
          setGrid((grid) => {
            ...
            for (const explosion of expired) {
              for (const [x, y] of explosion.coords) {
